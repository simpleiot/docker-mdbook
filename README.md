...
```bash
docker run --rm -v $(pwd):/book mdbook:0.5.1 build
```

Serve your book locally with live reload:

```bash
docker run --rm -v $(pwd):/book -p 3000:3000 mdbook:0.5.1 serve -n 0.0.0.0
```

Run link checking:

```bash
docker run --rm -v $(pwd):/book mdbook:0.5.1 test
```
